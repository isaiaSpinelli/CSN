...
#      28.11.2019   renommé en console_servo_usb.tcl
# ----------------------------------------------------------------------------------------
...
set consoleInfo(filename) "console_servo_usb"; # Filename without the filetype
...
proc CheckRunningMode {} {
  ...
  if {$err1 != "invalid command name \"restart\""} {
    set strResourcePath "../../../Console_ALU_Combi"
...
proc ReadInputs {} {
  ...
  if {$runningMode == "Simulation"} {
    ...
    foreach Seg $listSeg {
      set tmpSeg [examine  /Top_Sim/seg7_$Seg\_o]
      if {$tmpSeg == "U"} {
        set tmpSeg 0
      }
      if {$tmpSeg} {
